...
export const authApi = {
  ...
  lineTokenLogin: (accessToken: string) =>
    api.post("/auth/line/token-login", { accessToken }),
...
export const couponApi = {
  ...
  getCouponDetail: (assignmentId: string) =>
    api.get(`/coupons/my/${assignmentId}`),
  getAllCoupons: () => api.get("/coupons"), // Admin endpoint
  createCoupon: (data: any) => api.post("/coupons", data), // Admin create
};
...
export const redemptionApi = {
  validate: (couponCode: string, storeId?: string) =>
    api.post("/redemptions/validate", { couponCode, storeId }),
  redeem: (redemptionId: string, pointsEarned?: number) =>
    api.post("/redemptions/redeem", { redemptionId, pointsEarned }),
  getAll: (page = 1, limit = 20) =>
    api.get(`/redemptions?page=${page}&limit=${limit}`),
};
